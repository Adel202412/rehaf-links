...
const CONFIG = {
  ...
  i18n: {
    ...
    ar: {
      // Arabic UAE-local (clear, respectful, not corporate)
      ...
      <div class="rehaf-modal__header">
        <div class="rehaf-modal__brand">
          <div class="rehaf-modal__avatar" aria-hidden="true">
            <!-- You can switch this to /assets/logo.webp in your HTML/CSS -->
...
function render() {
  ...
  if (modal && modal.classList.contains("is-open") && currentModalKey) {
    // Re-open the same modal in the new language (keeps it consistent)
    // Keep values? For simplicity (fast UX), we refresh labels only:
    // close + open again to rebuild fields (cleaner than partial sync)
    // If you want to keep entered values, tell me and I’ll adjust.
