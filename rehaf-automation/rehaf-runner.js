...
// Public: renders card + boots game
export function renderRunner(container) {
  if (!container) return;
  container.innerHTML = `
    <div class="runner-card" id="runnerCard">
      <div class="runner-head">
        <div>🎮 <b>REHAF Runner</b> — auto-starts here. Tap or press <b>Space/↑/W</b> (double jump)</div>
        <div>Score: <span data-score>0</span></div>
      </div>
      <canvas width="820" height="240"></canvas>
      <div class="runner-help">Obstacles are E, H, A, F — slower, smoother physics.</div>
    </div>
  `;
  const card   = container.querySelector('#runnerCard');
  const canvas = container.querySelector('canvas');
  const score  = container.querySelector('[data-score]');
  initRehafRunner(canvas, score, card); // call the game engine below
}

// Game engine (kept same signature you already had)
export function initRehafRunner(canvas, scoreEl, card) {
  ...
  function toast(txt){ if(!card) return; const d=document.createElement('div'); d.className='toast'; d.textContent=txt; card.appendChild(d); setTimeout(()=>d.remove(),1600); }
  ...
  function drawRunner(){ ctx.save(); ctx.translate(R.x+R.w/2,R.y+R.h/2); ctx.rotate(R.angle); ctx.scale(R.scaleX,R.scaleY);
    ...
    const g=ctx.createLinearGradient(0,0,0,h); g.addColorStop(0,C.r1); g.addColorStop(1,C.r2); ctx.fillStyle=g; ctx.fill(); ctx.fillStyle='#fff'; ctx.font='bold 22px Inter, system-ui'; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('R', w/2, h/2-1);
    ctx.restore(); ctx.restore();
  }
  function drawObs(o){ const x=o.x,y=o.y; ctx.save(); ctx.fillStyle='rgba(0,171,154,0.08)'; ctx.strokeStyle='#cfe7e3'; ctx.lineWidth=1; ctx.fillRect(x-o.s/2,y-o.s,o.s,o.s); ctx.strokeRect(x-o.s/2,y-o.s,o.s,o.s); ctx.fillStyle=C.letter; ctx.font=`bold ${Math.floor(o.s*.8)}px Inter, system-ui`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText(o.ch,x,y-o.s/2+2); ctx.restore(); }
  function drawOver(){ if(running) return; ctx.save(); ctx.fillStyle='rgba(255,255,255,.85)'; ctx.fillRect(0,0,vw,vh); ctx.fillStyle=C.letter; ctx.font='bold 22px Inter, system-ui'; ctx.textAlign='center'; ctx.fillText('Game Over — tap or press Space to restart', vw/2, vh/2); ctx.restore(); }
